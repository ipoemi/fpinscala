...
input (`Await`) or signaling termination via `Halt`.
  ...
  sealed trait Process[I, O] {
    ...
    def map2[O2, O3](p: Process[I, O2])(f: (O, O2) => O3): Process[I, O3] =
      this flatMap (a => p map (b => f(a, b)))
    ...
    (this, p2) match {
      ...
      case (Halt(), Await(recv)) => this |> recv(None)
      case (Emit(h, t), Await(recv)) => t |> recv(Some(h))
      case (Await(recv), _) => Await {
        case None => recv(None) |> p2
        case i => recv(i) |> p2
      }
    }
    */
      p2 match {
        ...
        case Await(recv1) => this match {
          case Halt() => Halt() |> recv1(None)
          case Emit(h, t) => t |> recv1(Some(h))
          //case Await(recv2) => Await(recv2(_) |> p2)
    ...
    def zip[O2](p: Process[I, O2]): Process[I, (O, O2)] =
      (this, p) match {
        ...
        case (Await(recv), _) => Await(optA => recv(optA) zip Process.feed(optA)(p))
        case (_, Await(recv)) => Await(optA => Process.feed(optA)(this) zip recv(optA))
      }
  ...
  object Process {
    ...
    */
      if (n > 0) await[I, I](_ => drop(n - 1))
      else await[I, I](i => Emit(i, id))
    ...
    */
      await[I, I](i => if (f(i)) Emit(i, takeWhile(f)) else Halt())
    ...
    */
    ...
    def count[I]: Process[I, Int] = {
      def go(cnt: Int): Process[I, Int] = Await[I, Int] {
        case Some(i) => Emit(cnt + 1, go(cnt + 1))
        case _ => Halt()
      }
    ...
    def mean: Process[Double, Double] = {
      def go(acc: Double, cnt: Int): Process[Double, Double] = Await {
        case Some(i) => Emit((acc + i) / (cnt + 1), go(acc + i, cnt + 1))
        case _ => Halt()
      }
    ...
    def mean2: Process[Double, Double] = (sum zip count) map { case (s, c) => s / c }
    ...
    def zipWithIndex[I, O](p1: Process[I, O]): Process[I, (O, Int)] =
      p1 zipWithIndex
    ...
    def write = {
      ...
      val p = filter[String](_.trim.nonEmpty) |> filter(!_.startsWith("#")) |> lift(s => toCelsius(s.toDouble).toString)
      processFile(f1, p, new PrintWriter(new java.io.File("")))((pw, s) => {
        pw.write(s); pw
      })
    }
  }
    ...
    def runLog(implicit F: MonadCatch[F]): F[IndexedSeq[O]] = {
      ...
      def go(cur: Process[F, O], acc: F[IndexedSeq[O]]): F[IndexedSeq[O]] =
        cur match {
          ...
          case Await(req, recv) =>
            val next = F.attempt(req)
            F.flatMap(next)(x => go(Try(recv(x)), acc))
        }
      try go(this, F.unit(IndexedSeq()))
      catch {
        case err: Throwable => F.fail[IndexedSeq[O]](err)
      }
      */
...
`Process` to a `Sink`.
    ...
    def join[F[_], A](p: Process[F, Process[F, A]]): Process[F, A] = p.flatMap[A](identity)
...
object ProcessTest extends App {
  ...
  val p2 = eval(IO {
    ...
    case Kill => println {
      "cleanup was killed, instead of bring run"
    };
      Halt(Kill)
    case e => Halt(e)
  }
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val p1 = eval(IO{123}).map(_.toString)
    ...
    val p3 = p1.tee(p2)(zip)
    */

    val p2 = p1.to(fileW("test.txt"))

    val a = runLog(p2)

   // val aa = unsafePerformIO(a)(java.util.concurrent.Executors.newFixedThreadPool(4))
   // println(aa)
    println(test1())
