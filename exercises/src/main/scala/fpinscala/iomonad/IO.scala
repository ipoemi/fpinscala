...
object IO3 {
  ...
  */
  ...
  @annotation.tailrec
  def runTrampoline[A](mfa: Free[Function0, A]): A = mfa match {
    case Return(a) => a
    case Suspend(s) => s()
    case FlatMap(fa, f) => fa match {
      case Return(a) => runTrampoline(f(a))
      case Suspend(s) => runTrampoline(f(s()))
      case FlatMap(fb, g) => runTrampoline(fb flatMap (b => g(b) flatMap f))
    }
  }

  // Exercise 3: Implement a `Free` interpreter which works for any `Monad`
  def run[F[_], A](mfa: Free[F, A])(implicit F: Monad[F]): F[A] = step(mfa) match {
  ...
  // return either a `Suspend`, a `Return`, or a right-associated `FlatMap`
  // @annotation.tailrec
  def step[F[_], A](mfa: Free[F, A]): Free[F, A] = mfa match {
    case FlatMap(FlatMap(fa, g), f) => step(fa flatMap (a => g(a) flatMap f))
  ...
  */
  ...
  case object ReadLine extends Console[Option[String]] {
    ...
    def toReader = ConsoleReader { in => IO2b.Suspend(() => Some(in)) }
  }
  ...
  case class PrintLine(line: String) extends Console[Unit] {
    ...
    def toReader = ConsoleReader { s => IO2b.Return(()) } // noop
    def toState = ConsoleState { bufs => IO2b.Suspend(() => ((), bufs.copy(out = bufs.out :+ line))) } // append to the output
  }
  ...
  */
  ...
  case class ConsoleState[A](run: Buffers => IO2b.TailRec[(A, Buffers)]) {
    def map[B](f: A => B): ConsoleState[B] =
      ConsoleState { s => run(s).map(x => (f(x._1), x._2)) }

    def flatMap[B](f: A => ConsoleState[B]): ConsoleState[B] =
      ConsoleState { s =>
        run(s).flatMap(x => f(x._1).run(x._2))
      }
  }

  object ConsoleState {
    implicit val monad = new Monad[ConsoleState] {
      def unit[A](a: => A) = ConsoleState(bufs => IO2b.Return((a, bufs)))
  ...
  def read(file: AsynchronousFileChannel,
    ...
    Par.async { cb =>
      try {
        ...
        file.read(buffer, numBytes, null, new CompletionHandler[Integer, Any] {
          def failed(exc: Throwable, attachment: Any): Unit =
            cb(Left(exc))

          def completed(result: Integer, attachment: Any): Unit = {
            buffer.flip()
            cb(Right(buffer.array()))
          }
        })
      } catch {
        case e: Exception => e.printStackTrace(); cb(Left(e))
      }
    }
...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    val p = for {
      _ <- Console.printLn("테스트입니다. 숫자를 입력하세요.")
      s <- Console.readLn
      s2 = s.getOrElse("")
      _ <- Console.printLn(s2)
    } yield s + "1"

    //val f = runConsolePar(p).run(Buffers(List("123"), Vector()))
    val f = runConsole(p)
    println(f)
  }
}
